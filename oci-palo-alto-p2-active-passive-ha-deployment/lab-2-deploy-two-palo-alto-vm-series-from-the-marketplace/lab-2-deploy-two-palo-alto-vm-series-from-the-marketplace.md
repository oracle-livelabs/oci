...
With the VCN network in place, you now deploy the two Palo Alto VM-Series instances that will form the Active/Passive pair from Oracle Cloud Marketplace. Their management interfaces provide the access point for the PAN-OS setup in the next labs.
...
![Active/passive Palo Alto VM-Series deployment architecture](images/active-passive-palo-alto-vm-series-deployment-architecture.png)
...
| PA-VM-01 | `vnic-untrust`       | P: `172.16.0.20`<br>S: `172.16.0.22` | ethernet1/1         | P: `172.16.0.20/28`<br>S: `172.16.0.22/32` |
| PA-VM-01 | `vnic-trust`         | P: `172.16.0.40`<br>S: `172.16.0.42` | ethernet1/2         | P: `172.16.0.40/28`<br>S: `172.16.0.42/32` |
| PA-VM-01 | `vnic-ha`            | `172.16.0.50`                        | ethernet1/3         | `172.16.0.50`                              |
|          |                      |                                      |                     |                                            |
| PA-VM-02 | `vnic-management`    | `172.16.0.11`                        |                     | `172.16.0.11`                              |
| PA-VM-02 | `vnic-untrust`       | P: `172.16.0.21`<br>S: `172.16.0.22` | ethernet1/1         | P: `172.16.0.21/28`<br>S: `172.16.0.22/32` |
| PA-VM-02 | `vnic-trust`         | P: `172.16.0.41`<br>S: `172.16.0.42` | ethernet1/2         | P: `172.16.0.41/28`<br>S: `172.16.0.42/32` |
| PA-VM-02 | `vnic-ha`            | `172.16.0.51`                        | ethernet1/3         | `172.16.0.51`                              |
...
3. Click on the **Launch Instance** button.
    ...
    ![Specify first Palo Alto VM name](images/specify-name-instance.png)
...
2. Select your **shape**.
    ...
    > **Note:** Choose a shape configuration that supports at least four VNICs. For the `VM.Optimized3.Flex` shape used in this workshop, select at least two OCPUs; this supports up to four VNICs. For other shapes, confirm the VNIC limit in the [OCI Compute Shapes documentation](https://docs.oracle.com/en-us/iaas/Content/Compute/References/computeshapes.htm).
...
3. Click on the **Next** button.
    ...
    ![Verify first Palo Alto VM provisioning](images/wait-it-complete.png)

    - When the provisioning has been completed notice that the Instance is **Running** and the state is **Succeeded**.

    ![Verify first Palo Alto VM provisioning completed](images/when-provisioning-has-been-completed-notice-that-instance-is.png)
...
3. Notice the Private IPv4 address that you assigned.
    ...
    ![Verify PA-VM-01 IP addresses](images/notice-private-ipv4-address-that-you-assigned.png)
...
![Verify PA-VM-02 IP addresses](images/ip-address.png)
