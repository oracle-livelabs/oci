...
3. From the second terminal in VS Code, send multiple HTTP POST requests to the `/store/order` endpoint by running the following command 20 times:
...
4. Review the trace output in the Oracle Cloud Console. Navigate to the **Observability & Management**. Under **Application Performance Monitoring**, click **Trace Explorer**. Make sure the right compartment and domain are selected.
    ...
    You should see twenty new rows in the Traces list. You may need to refresh the page or change the selected duration ("Last 60 Minutes").

    ![Trace Explorer Twenty Requests](./images/trace-explorer-native-post-ten-requests.jpg#input)

    Click the most recent trace name to view the Trace Details. Each span is represented by a blue bar.

    ![Trace Details Twentieth Request](./images/trace-details-native-tenth-request.jpg#input)
